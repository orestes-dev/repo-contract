...
**Gate activation**:
The step that makes a vendored gate actually block a merge: its **Gate context** listed among the default branch's required status checks (classic protection or a ruleset). Distinct from the gate _running_, which vendoring the workflow already buys. The **Hook activation** split one layer up and for the same reason: both halves live in per-repo settings a repository cannot commit, so `init` ships the carrier and never the enforcement. Unactivated, a red gate blocks nothing and announces that nowhere, which is how a PR merged carrying `pr-readiness:failing` (orestes/dotfiles#84). Every hard-failing gate has one, not the PR gate alone: an unrequired `commit-hygiene` context blocks exactly as little as an unrequired `pr-readiness` one. `init` reports the gap per context and never repairs it (ADR 0014), since requiring a currently-red check blocks every open PR at once. The report keys off the vendored workflow file, never the `scaffolds` manifest, since the file is what makes a check run: that is what lets it name an **Orphan** gate running unrequired.
...
**Orphan**:
A **Scaffold**'s file present on disk while the `scaffolds` manifest does not list it: installed reality outrunning the record. `init` reports one and never removes it; `uninstall` resolves it. It arises where a repo scaffolded before the manifest existed runs a narrower `init --only`, which the never-deselect refusal cannot catch because the record it reads is empty. Detection reaches the filesystem and `core.hooksPath`, not the remote, because the question is whether the orphan is still enforcing: hooks `core.hooksPath` still points at fire on every commit and an orphaned gate's workflow still runs on every PR (so it gets a **Gate activation** verdict like any installed context), while its labels sit inert on the remote and are `uninstall`'s to name.
...
**Hook activation**:
The step that makes git actually invoke a **Vendored hook**: `core.hooksPath` pointing at `.repo-contract/hooks`, plus the hook file being executable. Distinct from execution (whether the hook can run once invoked), which is what vendoring and the hook's strict **Dependency budget** buy. `init` owns only the value it set: it sets `core.hooksPath` when this repo's local config leaves it unset, and leaves any other local value alone, the mirror of the way `uninstall` unsets only the `.repo-contract/hooks` it wrote and keeps anything else (ADR 0020). A foreign value (one repo-contract did not set: a stale `.husky`, an operator's own directory, an absolute path) is not repointed; because a git hook on disk that cannot be activated is inert, `init` refuses to write the **Git hooks** scaffold at all in that case and reports loudly, rather than laying down files it cannot turn on. The refusal names the remedy: unset the value and re-run, or opt in explicitly with `--overwrite-hooks-path` (or its interactive prompt), which repoints the value and prints the one it displaced, since a local `core.hooksPath` is never committed and has no reflog to recover it from. It also names what the operator does not lose: because the setting is single-valued, taking the slot displaces whatever held it, and those hooks keep running once their bodies move into the **`.repo-contract/hooks/local` chain**, the tool-agnostic adoption path repo-contract signposts instead of building a migration for any particular prior tool (ADR 0021). Activation is per-clone git config that no repository can commit, so one activation step per clone is the guarantee, and the **Commit hygiene** gate is the backstop for a checkout where nobody took it. The value repo-contract writes is always the relative `.repo-contract/hooks`, never absolute: `core.hooksPath` is shared across linked worktrees, so an absolute one pins every worktree to a single checkout's hooks while a relative one resolves against each worktree's own root (ADR 0012). The directory is namespaced under `.repo-contract/` rather than named `.husky` or `.githooks`, so a vendoring tool never claims a name a consumer may already own (ADR 0017).
...
**`.repo-contract.json`**:
The committed, repo-root file holding a repo's enforcement opt-outs, replacing the per-machine `git config hooks.*` that ADR 0002 retired as invisible and clone-losing. Plain JSON, parsed with no added dependency and left `jq`-queryable, which is what lets the hooks read it on their own **Dependency budget**. Its `overrides` map keys an opt-out to an `Override`: the `value` the check reads plus a required, non-empty `reason`. An absent file means full enforcement with no opt-outs, so a repo that never wrote it behaves exactly as before. The same file feeds the **Commit hygiene** CI gate, so a local relaxation and its CI mirror read one source. It also carries the **Scaffold** install manifest, a `scaffolds` array that is authoritative and rewritten on every `init` run. An absent key means none installed, not all-in, so a repo scaffolded before the manifest existed needs one `init` run to record what it already has; the array is never empty, and an unknown id throws rather than reading as a scaffold nobody installed and letting a later selection drop it unrefused. The manifest is neither a rule nor an opt-out, but a record of what `init` scaffolded; its authority stops at repo-contract's own decisions (see **Scaffold**).
...
**Dogfood instance**:
This repo's own installed copy of the `templates/` bundle, occupying the same paths a consumer's install occupies (`.github/`, the root `.template.{issue,pr}.md`, `.repo-contract/hooks/*`, and `.repo-contract.json`) and byte-identical to its source in every one, with no exception. The repo holds the **source** role too, but only in `templates/` and `src/`: the two roles live in different paths and never in the same file. That separation is what lets `init` run in its own repo and report every file `ok`, since `init`'s only vocabulary is byte-equality (ADR 0003). It was not always so: the three gate workflows were hand-authored `uses: ./` variants that also served as a pre-merge self-test of the action's own code, which made them permanently `stale` to `init` and needed a bespoke field-level comparator. ADR 0018 made them verbatim `@main` copies and dropped the self-test rather than relocating it, accepting that no real-API exercise of the action now happens before merge.
_Avoid_: Self-test, self-hosting, dogfooding (the practice, not this artifact).

**Drift test**:
A test asserting that a restated copy of a fact still matches its single source. The standing cases: each rendering's structure against its **Intent** (the Issue Form and the Author guides against `rules.js`, the PR template against `PR_SECTIONS`), the README threshold numbers against the rules, the gate job keys against `GATE_CONTEXT`, the workflow trigger filters against the label strings they hardcode, and the **Dogfood instance** against the canonical `templates/` bundle. That last one is a single table-driven assertion walking `SCAFFOLDS[].files`, since every installed destination is byte-identical to its source with no exception; a file is drift-checked the moment it joins the manifest, with no test to remember to write. Renderings are checked as strictly as their format allows: the YAML on headings, order, required, and options; the Markdown guides on headings and order only, since their prose is free. Duplication kept on purpose is made safe by a drift test rather than eliminated.

**Accepted duplication**:
A restatement deliberately left in place because collapsing it costs more than it saves, guarded by a drift test. Every instance is one of exactly two kinds: a **verbatim copy**, checked by exact equality (the **Dogfood instance**'s copies of the `templates/` bundle; the byte-identical PR pair `.template.pr.md` == `.github/PULL_REQUEST_TEMPLATE.md`, whose sameness is why PR guidance lives in HTML comments), or a **value restatement**, where a fact owned in code is repeated in a format that cannot import it (the README's threshold numbers against `rules.js`, the workflow job keys against `GATE_CONTEXT`, the trigger filters against the label strings). A partial overlap, two artifacts agreeing on some fields and legitimately differing on others, is no longer accepted anywhere: the workflow pair was the last one (consumer `@main` vs dogfood `./`), and ADR 0018 split the roles into separate paths rather than keep the field-level comparator it needed. Renderings of an **Intent** are a separate category, partial by design, not accepted duplication.
